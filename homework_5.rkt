...
;	която прилага подадената функция два пъти. Например, ако имаме функцията inc, 
;   която добавя 1 към своя аргумент ((define (inc x) (+ x 1)), то (double inc) е функция,
;   която добавя 2 към своя аргумент.
(define (double f)
    (lambda (x) (f (f x)))
)
