...
reverse' Nil = Nil
reverse' x = reverseHelp x Nil

reverseHelp :: List a -> List a -> List a
reverseHelp Nil acc = acc
reverseHelp (Cons x xs) acc = reverseHelp xs (Cons x acc)
...
maximum' Nil = undefined
maximum' (Cons x xs) = maximumH (Cons x xs) x

maximumH :: List Int -> Int -> Int
maximumH Nil x = x
maximumH (Cons x xs) y = maximumH xs (myIf (x > y) x y)
